 ...
 // execute searches when the search form is submitted
  ...
  jQuery(document).ready(function(event) {
    ...
    * Copyright (c) 2016 Dmitry Semenov; */
    ...
    jQuery(".showcase-caption a").on("click", function(e) {
      e.stopPropagation();
    });

    jQuery(".showcase-caption").on("click", function(event) {

      var items = [];
      jQuery(this).closest(".showcase-gallery").children("div").each(function() {
        items.push( {
          src: jQuery(this).find("a").first().attr("href"),
          titleSrc: jQuery(this).find(".showcase-caption").html()
        } );
      });
    ...
    jQuery(".showcase-gallery img").each(function() {
      var proportion = jQuery(this).height() / jQuery(this).width();
      var currentWidth = jQuery(this).parent().parent().width();
      if (proportion > 1) {
        jQuery(this).css("width", currentWidth);
      } else {
        var cssTextVar = "height: " + currentWidth + "px !important";
        jQuery(this).css("cssText", cssTextVar);
      }
    });
...
})(this, this.document);// show/hide element */
